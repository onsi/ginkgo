...
var _ = Describe("Coverage Specs", func() {
	...
	FIt("runs coverage analysis on external packages in series and in parallel", func() {
		...
		Eventually(session).Should(gexec.Exit(0))
		output := session.Out.Contents()
		Ω(output).Should(ContainSubstring("coverage: 57.1% of statements in github.com/onsi/ginkgo/integration/_fixtures/coverage_fixture, github.com/onsi/ginkgo/integration/_fixtures/coverage_fixture/external_coverage_fixture"))
