...
var _ = Describe("ReportEntries", func() {
	...
	It("should honor the report visibilities ans tringer formatting when emitting output", func() {
	...
	It("captures all report entries in the JSON report", func() {
		...
		Ω(passes.ReportEntries[0].Value).Should(Equal(map[string]interface{}{"Label": "pass-bob", "Count": float64(1)}))
		Ω(passes.ReportEntries[0].Representation).Should(Equal("{{red}}pass-bob {{green}}1{{/}}"))
		...
		Ω(passes.ReportEntries[1].Value).Should(BeNil())
		Ω(passes.ReportEntries[1].Representation).Should(BeZero())

		Ω(passes.ReportEntries[2].Name).Should(Equal("passes-third-report"))
		Ω(passes.ReportEntries[2].Value).Should(Equal(float64(3)))
		Ω(passes.ReportEntries[2].Representation).Should(BeZero())

		Ω(passes.ReportEntries[3].Name).Should(Equal("passes-pointer-report"))
		Ω(passes.ReportEntries[3].Value).Should(Equal(map[string]interface{}{"Label": "passed", "Count": float64(4)}))
		Ω(passes.ReportEntries[3].Representation).Should(Equal("{{red}}passed {{green}}4{{/}}"))

		Ω(passes.ReportEntries[4].Name).Should(Equal("passes-failure-report"))
		Ω(passes.ReportEntries[4].Value).Should(Equal(float64(5)))
		Ω(passes.ReportEntries[4].Representation).Should(BeZero())

		Ω(passes.ReportEntries[5].Name).Should(Equal("passes-never-see-report"))
		Ω(passes.ReportEntries[5].Value).Should(Equal(float64(6)))
		Ω(passes.ReportEntries[5].Representation).Should(BeZero())

		fails := reports.Find("fails")
		Ω(fails.ReportEntries[0].Name).Should(Equal("fails-first-report"))
		Ω(fails.ReportEntries[0].Value).Should(Equal(map[string]interface{}{"Label": "fail-bob", "Count": float64(1)}))
		Ω(fails.ReportEntries[0].Representation).Should(Equal("{{red}}fail-bob {{green}}1{{/}}"))
		...
		Ω(fails.ReportEntries[1].Value).Should(BeNil())
		Ω(fails.ReportEntries[1].Representation).Should(BeZero())

		Ω(fails.ReportEntries[2].Name).Should(Equal("fails-third-report"))
		Ω(fails.ReportEntries[2].Value).Should(Equal(float64(3)))
		Ω(fails.ReportEntries[2].Representation).Should(BeZero())

		Ω(fails.ReportEntries[3].Name).Should(Equal("fails-pointer-report"))
		Ω(fails.ReportEntries[3].Value).Should(Equal(map[string]interface{}{"Label": "failed", "Count": float64(4)}))
		Ω(fails.ReportEntries[3].Representation).Should(Equal("{{red}}failed {{green}}4{{/}}"))

		Ω(fails.ReportEntries[4].Name).Should(Equal("fails-failure-report"))
		Ω(fails.ReportEntries[4].Value).Should(Equal(float64(5)))
		Ω(fails.ReportEntries[4].Representation).Should(BeZero())
