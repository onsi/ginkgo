...
var _ = Describe("DefaultReporter", func() {
	...
	DescribeTable("WillRun",
		...
		Entry("ReportAfterSuite nodes", C(Verbose),
			S(CTS("my report"), CLS(cl0), types.NodeTypeReportAfterSuite, cl0),
			DELIMITER,
	...
	DescribeTable("DidRun",
		...
		Entry("a passing ReportAfterSuite emits nothing",
			C(),
			S(CTS("my report"), CLS(cl0), types.NodeTypeReportAfterSuite, cl0, GW("GINKGO-WRITER-OUTPUT")),
		),
		Entry("a passing ReportAfterSuite with verbose always emits",
			...
			"{{green}}{{bold}}[ReportAfterSuite] my report{{/}}",
			"{{gray}}"+cl0.String()+"{{/}}",
			"",
			"  {{gray}}Begin Captured GinkgoWriter Output >>{{/}}",
			"    GINKGO-WRITER-OUTPUT",
		...
		Entry("a passing ReportAfterSuite with captured stdout always emits",
			C(),
			S(CTS("my report"), CLS(cl0), types.NodeTypeReportAfterSuite, cl0, STD("STD-OUTPUT")),
			DELIMITER,
			"{{green}}[ReportAfterSuite] PASSED [1.000 seconds]{{/}}",
			"{{green}}{{bold}}[ReportAfterSuite] my report{{/}}",
			"{{gray}}"+cl0.String()+"{{/}}",
		...
		Entry("when a ReportAfterSuite node has failed",
			C(),
			S(CTS("my report"), CLS(cl0), types.NodeTypeReportAfterSuite, cl0, types.SpecStateFailed, 1,
				GW("GW-OUTPUT\nIS EMITTED"), STD("STD-OUTPUT\nIS EMITTED"),
				F("FAILURE MESSAGE\nWITH DETAILS", Location(cl3), types.NodeTypeReportAfterSuite, 0),
			),
			DELIMITER,
			...
			"  {{red}}FAILURE MESSAGE",
			"  WITH DETAILS{{/}}",
			"  {{red}}In {{bold}}[ReportAfterSuite]{{/}}{{red}} at: {{bold}}"+cl3.String()+"{{/}}",
			DELIMITER,
			"",
		),
	...
	DescribeTable("Rendering SuiteDidEnd",
		...
		Entry("when the suite includes a special failure reason",
			...
			types.Report{
				SuiteSucceeded:            false,
				SpecialSuiteFailureReason: "Detected pending specs and --fail-on-pending is set",
